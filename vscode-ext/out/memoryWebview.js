...
class MemoryViewProvider {
    ...
    async _getDoubleBufferInfo(session, frameId, blockNames) {
        const result = {};
        ...
        if (!countR || countR.result === undefined)
            return result;
    ...
    async _getPoolNames(session, frameId) {
        // Find PoolAllocator* variables via GDB
        {
            ...
            if (r && r.result) {
                ...
                if (names.length > 0) {
                    console.log('[Brick Mem] Pool names:', [...new Set(names)]);
                    return [...new Set(names)];
                }
            }
        }
    ...
    async _readPoolsFromSession(session) {
        ...
        for (const name of names) {
            try {
                ...
                if (!slotCountR || slotCountR.result === undefined)
                    continue;
